...
//go:embed embedded/GeoLite2-Country.mmdb
...
// main функция для демонстрации работы роутера трафика
func main() {
	// Создаем роутер трафика
	router, err := NewTrafficRouter("")
	if err != nil {
	...
	if len([]string{}) > 1 { // В реальном приложении будет os.Args
		ip := "77.88.8.8" // Пример IP для тестирования
		isRU := router.IsRUDestination(ip)
		
		if isRU {
			fmt.Printf("IP %s является РУ трафиком - направлять напрямую\n", ip)
		} else {
			fmt.Printf("IP %s является неРУ трафиком - направлять через VPN\n", ip)
		}
		
		// Получаем полное решение
		decision := router.GetRouteDecision(ip)
		fmt.Printf("Подробное решение: %+v\n", decision)
		
		return
	}

	// Запуск HTTP API
	log.Println("Запуск HTTP API роутера трафика...")
	if err := router.StartHTTPServer("8081"); err != nil {
