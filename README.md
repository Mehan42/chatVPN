...
1. Локация - Россия, провайдер открыл порты 80, 443 другие закрыты наглухо.
2. Трафик должен разделяться на ру и неру сегмент, ру сегмент напрямую, неру сегмент через тоннель на удалённый сервер. Сортировка на стороне локального пользователя.
3. Провайдер давит прокси в любом виде, поэтому многие протоколы не работают, Амнезия ВПН блокируется на лету.
4. При установлении соединения нужен скрипт у локального пользователя который пингует удалённый сервер и при попытках блокировки автоматически переходит на другой протокол передачи трафика.
5. Трафик на удалённом серврере может распределяться по сети через MCP и собираться обратно тем же способом, но приходить к пользователю через тот же порт что и был отправлен. Провайдер не должен видеть ни чего подозрительного и не иметь повода к блокировке.
