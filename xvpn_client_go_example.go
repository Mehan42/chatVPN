...
func (sm *VPNStateMachine) handleEvent(event Event) {
	...
	transition, exists := sm.transitionMap[currentState][event]
...
func (sm *VPNStateMachine) actionConfigFetching() {
	...
	transports := sm.context.Config["transports"].([]map[string]interface{})
	for _, t := range transports {
		transport := Transport{
			ID:       t["id"].(string),
			Name:     t["name"].(string),
			Type:     t["type"].(string),
			Priority: int(t["priority"].(float64)),
		}
...
// main функция для демонстрации работы машины состояний
func main() {
	clientUUID := "test-client-uuid-12345"
	...
	// Для примера, триггерим несколько событий
	go func() {
		time.Sleep(2 * time.Second)
		stateMachine.TriggerEvent(EventStartRequested)
		
		time.Sleep(5 * time.Second)
		stateMachine.TriggerEvent(EventHealthCheckFailed)
		
		time.Sleep(3 * time.Second)
		stateMachine.TriggerEvent(EventStopRequested)
	}()
	
	// Ждем 15 секунд и останавливаем
	time.Sleep(15 * time.Second)
	stateMachine.Stop()
	
	fmt.Println("State machine stopped")
	fmt.Printf("Final state: %s\n", stateMachine.context.CurrentState)
	fmt.Printf("State history: %v\n", stateMachine.stateHistory)
	
	// Выводим информацию о состоянии
	info, _ := json.MarshalIndent(stateMachine.GetStateInfo(), "", "  ")
	fmt.Printf("Final state info: %s\n", info)
}