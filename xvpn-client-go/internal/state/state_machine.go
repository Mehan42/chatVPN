...
// UpdateConfig обновляет конфигурацию машины состояний
func (sm *VPNStateMachine) UpdateConfig(transports []Transport) {
	sm.mutex.Lock()
	defer sm.mutex.Unlock()
	
	// Обновляем список fallback транспортов
	sm.context.FallbackTransports = transports
	sm.logger.Printf("Конфигурация обновлена, транспортов: %d", len(transports))
}
...
func (sm *VPNStateMachine) actionError() {
	...
	if sm.context.ErrorCount < sm.context.MaxRetries {
		...
		// Увеличиваем задержку между попытками с ограничением максимального времени
		baseDelay := time.Duration(sm.context.ErrorCount*2) * time.Second
		maxDelay := 30 * time.Second  // Максимальная задержка 30 секунд
		actualDelay := baseDelay
		if actualDelay > maxDelay {
			actualDelay = maxDelay
		}
		
		sm.logger.Printf("Waiting %v before retry...", actualDelay)
		time.Sleep(actualDelay) // экспоненциальный откат с ограничением
		sm.TriggerEvent(EventStartRequested)
	} else {
